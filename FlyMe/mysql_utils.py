...
def check_airplane_capacity(mtplx_cnx, flight_number, airline):
    try:
        cursor = mtplx_cnx.cursor()
...
def _generate_ticket(mtplx_cnx, ticket_id, airline, flight_number):
    try:
        check = check_airplane_capacity(mtplx_cnx, flight_number, airline)
        cursor = mtplx_cnx.cursor()
        if check == 'full':
            return 'full'
...
def view_my_flight_customer(cnx, email, location, start_date, end_date):
    try:
        ...
        if not location and not start_date and not end_date:
            ...
            cursor.execute(
                "SELECT * FROM flight WHERE (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases where customer_email = %s) and arrival_time <= date_add(%s, interval 1 day)",
                (email, end_date))
            ...
            if location in cities:
                cursor.execute(
                    "select * from flight where (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases where customer_email = %s) and (departure_airport in (select airport_name from airport where airport_city = %s) or arrival_airport in (select airport_name from airport where airport_city = %s)) and arrival_time <= date_add(%s, interval 1 day)",
                    (email, location, location, end_date))
            else:
                cursor.execute(
                    "select * from flight where (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases where customer_email = %s) and (departure_airport = %s or arrival_airport = %s) and arrival_time <= date_add(%s, interval 1 day)",
                    (email, location, location, end_date))
        elif not location and start_date and end_date:
            cursor.execute(
                "SELECT * FROM flight WHERE (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases where customer_email = %s) and %s <= departure_time and arrival_time <= date_add(%s, interval 1 day) ",
                (email, start_date, end_date))
        elif location and start_date and end_date:
            if location in cities:
                cursor.execute(
                    "select * from flight where (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases where customer_email = %s) and (departure_airport in (select airport_name from airport where airport_city = %s) or arrival_airport in (select airport_name from airport where airport_city = %s)) and departure_time >= %s and arrival_time <= date_add(%s, interval 1 day)",
                    (email, location, location, start_date, end_date))
            else:
                cursor.execute(
                    "select * from flight where (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases where customer_email = %s) and (departure_airport = %s or arrival_airport = %s) and departure_time >= %s and arrival_time <= date_add(%s, interval 1 day)",
                    (email, location, location, start_date, end_date))
        result = []
        dic = cursor.fetchone()
        while dic is not None:
...
def view_my_flight_booking_agent(cnx, email, location, start_date, end_date):
    try:
        ...
        if not location and not start_date and not end_date:
            ...
            cursor.execute(
                "SELECT * FROM flight WHERE (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases natural join booking_agent where email = %s) and arrival_time <= date_add(%s, interval 1 day)",
                (email, end_date))
        elif location and start_date and not end_date:
            if location in cities:
                cursor.execute(
            ...
            if location in cities:
                cursor.execute(
                    "select * from flight where (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases natural join booking_agent where email = %s) and (departure_airport in (select airport_name from airport where airport_city = %s) or arrival_airport in (select airport_name from airport where airport_city = %s)) and arrival_time <= date_add(%s, interval 1 day)",
                    (email, location, location, end_date))
            else:
                cursor.execute(
                    "select * from flight where (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases natural join booking_agent where email = %s) and (departure_airport = %s or arrival_airport = %s) and arrival_time <= date_add(%s, interval 1 day)",
                    (email, location, location, end_date))
        elif not location and start_date and end_date:
            cursor.execute(
                "SELECT * FROM flight WHERE (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases natural join booking_agent where email = %s) and %s <= departure_time and arrival_time <= date_add(%s, interval 1 day) ",
                (email, start_date, end_date))
        elif location and start_date and end_date:
            if location in cities:
                cursor.execute(
                    "select * from flight where (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases natural join booking_agent where email = %s) and (departure_airport in (select airport_name from airport where airport_city = %s) or arrival_airport in (select airport_name from airport where airport_city = %s)) and departure_time >= %s and arrival_time <= date_add(%s, interval 1 day)",
                    (email, location, location, start_date, end_date))
            else:
                cursor.execute(
                    "select * from flight where (flight_num,airline_name) in (select flight_num,airline_name from ticket natural join purchases natural join booking_agent where email = %s) and (departure_airport = %s or arrival_airport = %s) and departure_time >= %s and arrival_time <= date_add(%s, interval 1 day)",
                    (email, location, location, start_date, end_date))
        result = []
        dic = cursor.fetchone()
        while dic is not None:
...
def add_flight(cnx, airline_staff_username, flight_num, departure_airport, departure_time, arrival_airport,
    ...
    try:
        ...
        # list_of_dic = cursor.fetchall()
        try:
            staff_airline = cursor.fetchall()[0]['airline_name']
        except:
            return {'good': False,
                    'error': "Airline name not matched."}
...
def get_airplane(cnx, airline_staff_username):
    try:
        ...
        logger.error(str(ex))
        return {'good': False, 'error': str(ex)}
    finally:
        cursor.close()
        cnx.close()
...
def view_top_booking_agents(cnx, airline_staff_username):
    try:
        ...
        if result3 and len(result3) > 5:
            result3 = result3[0:5]
        return {'ticket_year': result1, 'ticket_month': result2, 'commission': result3}
    except pymysql.Error as ex:
        logger.error(str(ex))
        return {}
    finally:
...
def view_report(cnx, airline_staff_username, start_date, end_date):
    try:
        cnx = pymysql.connect(**cnx)
        cursor = cnx.cursor()
        cursor.execute("select airline_name from airline_staff where username = %s", (airline_staff_username,))
...
def get_all_cities_and_airports(cnx):
    try:
        ...
        result = []
        while dic1 is not None:
            result.append(dic1['airport_name'])
            dic1 = cursor.fetchone()
        cursor.execute("select distinct airport_city from airport")
        dic2 = cursor.fetchone()
        while dic2 is not None:
            result.append(dic2['airport_city'])
            dic2 = cursor.fetchone()
        return result
    except pymysql.Error as ex:
        logger.error(str(ex))
